...
class UnitPolicy
{
    ...
    public function view(?User $user, Module $module): bool
    {
        ...
        if (in_array($user->role->name, [Roles::HEAD_TEACHER->value, Roles::TEACHER->value])) {
            $user_has_access_to_unit = ModulesTeacher::where('module_id', $module->id)->where('teacher_id', $user->id)
                ->first();
        } else if ($user->role->name == Roles::STUDENT->value) {
            $user_has_access_to_unit = ModulesStudent::where('module_id', $module->id)->where('student_id', $user->id)
                ->first();
        }
