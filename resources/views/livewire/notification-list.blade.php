        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex flex-col space-y-3">
                @forelse ($user_notifications as $notification)
                    @if (array_key_exists('announcement', $notification))
                        {{-- TODO:
                        ...
                        <a
                            ...
                            wire:navigate
                            href="{{ route('announcements.detail', ['id' => $notification['announcement']['id'], 'slug' => $notification['announcement']['slug']]) }}"
                            ...
                            <div class="flex justify-between gap-3">
                                <div class="flex flex-1 flex-col">
                                    ...
                                    <p class="indent-1 text-gray-800 dark:text-gray-200">
                                        {{ strip_tags(\Illuminate\Support\Str::limit($notification['announcement']['description'], '128', ' (...)')) }}
                                    </p>
                    ...
                    @elseif (array_key_exists('meeting', $notification))
                        <a
                            wire:click.prevent="set_is_read('{{ $notification['id'] }}')"
                            wire:navigate
                            href="{{ route('meetings.detail', ['meeting_uuid' => $notification['meeting']['meeting_uuid']]) }}"
                            class="inline-block w-full text-sm p-2 border-b border-gray-200 dark:border-gray-900 first-of-type:rounded-t-md
                            {{ $notification['read_at'] ? 'bg-gray-100/50 dark:bg-gray-800/50 hover:bg-gray-200 dark:hover:bg-gray-700' : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600' }}"
                        >
                            <div class="flex justify-between gap-3">
                                <div class="flex flex-1 flex-col">
                                    ...
                                    <p class="indent-1 text-gray-800 dark:text-gray-200">
                                        ...
                                        @elseif (Auth::user()->role->name == \App\Enums\Roles::STUDENT->value)
                                            You have booked this meeting. View your meeting details by clicking this notification.
                                        @endif
                                    </p>
                                ...
                                <div class="flex items-center">
                                    <x-custom.unread-circle :is_read="$notification['read_at']" :tooltip="'Unread meeting'" />
                                </div>
                            </div>
                        </a>
