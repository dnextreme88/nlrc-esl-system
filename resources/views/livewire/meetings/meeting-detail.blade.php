<div>
    ...
    <div class="grid grid-cols-1 gap-8 mx-auto space-y-6 max-w-7xl px-4 py-12 sm:px-6 lg:px-8 {{ $current_meeting->status == \App\Enums\MeetingStatuses::PENDING->value ? 'lg:space-y-0 lg:grid-cols-[1fr_minmax(10%,_36%)]' : '' }}">
                        ...
                        <div class="relative md:mx-auto">
                            ...
                            <div class="size-6 absolute top-1/2 -mt-2 rounded-full
                                @if ($is_last_element)
                                    ...
                                    @if ($current_meeting->status == \App\Enums\MeetingStatuses::PENDING->value)
                                        animate-pulse
                                    @endif
                                @else
        ...
        @if ($current_meeting->status == \App\Enums\MeetingStatuses::PENDING->value)
            <div class="p-4 mt-10 flex flex-col space-y-6 border-2 border-gray-300 dark:border-gray-600 shadow-md shadow-gray-500">
                ...
                <p class="text-gray-800 dark:text-gray-200">Time: <span class="font-bold">{{ Helpers::parse_time_to_user_timezone($current_meeting['start_time'])->format('g:i A') }} ~ {{ Helpers::parse_time_to_user_timezone($current_meeting['end_time'])->format('g:i A') }}</span></p>

                @if ($current_meeting['meeting_link'])
